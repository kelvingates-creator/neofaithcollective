import { N as NAVBAR_HEIGHT, w as widgetManager } from './grid_7BbutKON.mjs';
...
class TableOfContents extends HTMLElement {
  ...
  headingIdxMap = /* @__PURE__ */ new Map();
  headings = [];
  tocEntries = [];
  active = [];
  activeIndicator = null;
  _retryCount = 0;
  _backToTopObserver = null;
  _handleBtnClick = (e) => {
    e.stopPropagation();
  ...
  markVisibleSection = (entries) => {
    entries.forEach((entry) => {
      ...
      const idx = id ? this.headingIdxMap.get(id) : void 0;
      if (idx != void 0)
  ...
  isPostPage() {
    return window.location.pathname.includes("/posts/") || document.querySelector(".custom-md, .markdown-content") !== null;
  }
  updateFloatingPosition = () => {
    if (this.dataset.isFloating !== "true") return;
    const container = this.querySelector(".toc-floating-container");
  ...
  regenerateTOC() {
    ...
    const tocWrapper = isFloating ? this.querySelector(".toc-floating-container") : this.closest("widget-layout");
    ...
    let heading1Count = 1;
    const tocHTML = headings.filter((heading) => heading.depth < minDepth + maxLevel).map((heading) => {
      const depthClass = heading.depth === minDepth ? "" : heading.depth === minDepth + 1 ? "ml-4" : "ml-8";
      const badgeContent = heading.depth === minDepth ? heading1Count++ : heading.depth === minDepth + 1 ? '<div class="transition w-2 h-2 rounded-[0.1875rem] bg-(--toc-badge-bg)"></div>' : '<div class="transition w-1.5 h-1.5 rounded-xs bg-black/5 dark:bg-white/10"></div>';
  ...
  init() {
    ...
    const allEntries = Array.from(this.querySelectorAll("a[href^='#']"));
    const validHeadings = [];
    const validEntries = [];
  ...
  connectedCallback() {
    ...
    const setupSwup = () => {
      if (window.swup && window.swup.hooks) {
        if (this._swupListenersAdded) return;
